...
resource "null_resource" "download-extract-image-freebsd-release-12-aarch64" {
  count = var.enable_freebsd_120_release_aarch64_qcow2 || var.enable_freebsd_120_release_aarch64_raw ? 1:0
  provisioner "local-exec" {
    command = "${path.module}/freebsd_aarch64_image.sh"
  }
}
resource "openstack_images_image_v2" "freebsd-120-release-aarch64-raw" {
  count = var.enable_freebsd_120_release_aarch64_raw ? 1:0
  name   = "freebsd-12.0-release-aarch64-raw"
...
resource "openstack_images_image_v2" "freebsd-120-release-aarch64-qcow2" {
  count = var.enable_freebsd_120_release_aarch64_qcow2 ? 1:0
  name   = "freebsd-12.0-release-aarch64-qcow2"
